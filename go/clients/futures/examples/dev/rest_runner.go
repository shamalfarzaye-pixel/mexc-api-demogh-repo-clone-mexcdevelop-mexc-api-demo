// Program rest-runner runs a single REST action via actionrunner.RunAction.
// Edit target below, then: go run ./clients/futures/examples/dev/rest_runner.go
// Loads .env from parent directories when present.
package main
...
import (
	...
	"github.com/mexcdevelop/mexc-api-demo/go/clients/futures/internal/actionrunner"
)

// Target configures which REST action to run and its parameters.
// Param rules and supported actions are defined in actionrunner.RunAction / ListActions.
type Target struct {
	Action string
...
func main() {
	...
	resp, err := actionrunner.RunAction(ctx, client, target.Action, target.Params)
...
func printResult(name string, resp *futures.RawResponse, err error) {
	fmt.Printf("==== %s ====\n", name)
	if resp != nil {
	...
	if err != nil {
		if e, ok := err.(*futures.APIError); ok {
