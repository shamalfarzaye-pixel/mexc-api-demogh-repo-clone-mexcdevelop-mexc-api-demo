// Dev-only action runner for examples/dev/rest_runner.go — not part of the public SDK API.
...
type actionHandler func(ctx context.Context, c *futures.RestAPI, params map[string]any) (*futures.RawResponse, error)
...
func RunAction(ctx context.Context, c *futures.RestAPI, action string, params map[string]any) (*futures.RawResponse, error) {
	if action == "" {
		return nil, fmt.Errorf("action required")
	}
	if privateActions[action] && !c.HasAuth() {
...
func klineParamsFromMap(params map[string]any) futures.KlineParams {
	return futures.KlineParams{
